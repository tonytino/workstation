...
`bootstrap.sh` is an idempotent 12-stage installer that takes a fresh macOS to a working development state. Each stage is a separate helper script under `scripts/`.
...
5. **chezmoi apply** — renders templates from `home/` (per `.chezmoiroot`) into `$HOME`. Templates that reference `op://...` will fail if stage 3 didn't authenticate.
6. **Brewfile** — `brew bundle` against `macos/Brewfile`.
7. **macOS defaults** — `macos/defaults.sh`, all curated and reversible.
8. **GitHub auth** — `gh auth login` (or `gh auth refresh`) with `admin:public_key` scope so stage 9 can register the SSH key.
...
12. **Manual follow-ups checklist** — print remaining human-click items.
...
3. Add a `banner "N/M ..."` block + `bash "${SCRIPTS}/your-script.sh"` invocation at the right ordering point in `bootstrap.sh`.
4. Renumber surrounding banners (`N/M`) so they reflect the new stage count.
5. If the stage produces a manual follow-up, append a bullet to the stage-11 heredoc in `bootstrap.sh`.
6. Update the README's stage list.
